...
#define DT_DRV_COMPAT zmk_behavior_p2sm_scroll_sens
LOG_MODULE_DECLARE(zmk, CONFIG_ZMK_LOG_LEVEL);
#if DT_HAS_COMPAT_STATUS_OKAY(DT_DRV_COMPAT)

struct behavior_p2sm_scroll_sens_config {
    uint32_t step;
    uint32_t max_multiplier;
};

static int p2sm_detect_drift(const struct zmk_behavior_binding *binding) {
    const struct behavior_p2sm_scroll_sens_config *cfg = zmk_behavior_get_binding(binding->behavior_dev)->config;
    const float current = p2sm_get_yaw_coef();
...
static int on_p2sm_binding_pressed(struct zmk_behavior_binding *binding, struct zmk_behavior_binding_event event) {
    ...
    const float current = p2sm_get_yaw_coef();
    const float max_value = cfg->max_multiplier * 1.0f;
    ...
    if (new_val > max_value || new_val < 0.0f) {
        new_val = fmod(new_val, max_value);
        if (new_val < 0.0f) {
            new_val = new_val + max_value;
        }
...
static int behavior_p2sm_scroll_sens_init(const struct device *dev) {
    p2sm_detect_drift(zmk_behavior_get_binding(dev->name)->config);
    return 0;
}

static const struct behavior_driver_api behavior_p2sm_scroll_sens_driver_api = {
...
static const struct behavior_p2sm_scroll_sens_config behavior_p2sm_scroll_sens_config = {
    .step = DT_INST_PROP(0, step),
    .max_multiplier = DT_INST_PROP_OR(0, max_multiplier, 1),
};
BEHAVIOR_DT_INST_DEFINE(0, behavior_p2sm_scroll_sens_init, NULL, NULL, &behavior_p2sm_scroll_sens_config,
    POST_KERNEL, CONFIG_KERNEL_INIT_PRIORITY_DEFAULT, &behavior_p2sm_scroll_sens_driver_api);
